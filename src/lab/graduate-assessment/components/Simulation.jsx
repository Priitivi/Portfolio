...
import { answerLabels, questionRationale } from "../engine/questions.js";
import {
  assembleSimulation,
...
function SimulationReview({ completed, onExit, onRestart }) {
  ...
  return (
    <main className="ga-main ga-simulation-results">
      <section className="ga-result-hero">
        ...
        <div className="ga-result-actions"><button type="button" className="ga-button ga-button-primary" onClick={onExit}>Return to dashboard</button><button type="button" className="ga-button ga-button-ghost" onClick={onRestart}>New simulation</button></div>
      </section>
      ...
      <section className="ga-review-panel">
        <div className="ga-panel-heading">
          <div><span>END-OF-ASSESSMENT REVIEW</span><h2>Question {reviewIndex + 1} of {questions.length}</h2></div>
        ...
        <div className="ga-review-answer">
          ...
          <p>{questionRationale(question, answer?.selected ?? -1)}</p>
...
export default function Simulation({ checkpoint, selectionContext, onCheckpoint, onComplete, onExit }) {
  ...
  if (completed) return <SimulationReview completed={completed} onExit={onExit} onRestart={() => setCompleted(null)} />;
